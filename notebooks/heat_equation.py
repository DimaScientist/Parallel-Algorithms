...
def get_system_matrix(num: int, lambda_value: float) -> np.array:
    ...
    format_matrix(matrix, num)

    return matrix


def format_matrix(array: np.array, size: int) -> None:
    """Возвращает матрицу в трехдиагональной форме для упрощенного решения СЛАУ."""
    for i in range(1, size - 1):

        if array[1 + 3 * (i - 1)] == 0:
            return

        array[2 + 3 * (i - 1)] /= array[1 + 3 * (i - 1)]
        array[1 + 3 * i] -= array[2 + 3 * (i - 1)] * array[3 * i]
